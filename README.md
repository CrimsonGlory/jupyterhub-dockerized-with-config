...
- **Renew / deploy-hook errors after upgrading this repo**: older configs may have stored a deploy hook that called `curl`, or a shell snippet that broke Certbot’s renewal INI parser. New hooks use the executable shell wrapper `/usr/local/lib/certbot-hooks/deploy-nginx-reload.sh`, which runs `reload-via-docker-socket.py`. Orphan renewal files and renewal files that Certbot cannot parse (detected with `certbot renew --cert-name … --dry-run`) are removed automatically before each `certbot renew`.
- **Lineage `your.domain-0001` while nginx points at `live/your.domain/`**: usually caused by leaving bootstrap files in `live/your.domain/` when the first `certonly` ran. New entrypoint logic clears unmanaged `live/<name>/` before `certonly` so the standard name is used. If you already have `-0001` only, either point `ssl_certificate` / `ssl_certificate_key` in `nginx.conf` at `live/<name>-0001/`, or `docker compose down`, delete the `letsencrypt` named volume, and bring the stack up again. A NOTE about this is printed only when that mismatch is true (not after nginx paths already use `-0001`).
